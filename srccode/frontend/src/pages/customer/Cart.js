...
function Cart() {
  ...
  const getItemId = (item) => item.foodId || item.id;

  const getItemName = (item) => item.foodName || item.name || item.title || 'Món ăn';

  const getItemImage = (item) => item.img || item.emoji || '🍽️';
  ...
  const handleOrder = async () => {
    ...
    try {
      ...
      if (payMethod !== 'cod') {
        const paymentResponse = await API.post(`/orders/${response.data.orderId}/payment/confirm`, {
          paymentMethod: payMethod.toUpperCase(),
          amount: finalTotal,
          providerTransactionId: `${payMethod.toUpperCase()}-${Date.now()}`
        });

        orderData = paymentResponse.data;
      }
      ...
      <div className="order-success">
        ...
        <div className="success-info-box">
          ...
          <div className="success-row">
            ...
            <strong>{createdOrder?.paymentStatus || (payMethod === 'cod' ? 'UNPAID' : 'PENDING')}</strong>
          </div>
          ...
          <div className="success-row">
            ...
            <strong style={{ color: '#e85d04' }}>
              {(createdOrder?.paidAmount || finalTotal).toLocaleString('vi-VN')}đ
            </strong>
          </div>
