  ...
  const handleConfirm = async () => {
    ...
    try {
      const reservation = await createReservation({
        ...
        note: form.note,
        preOrderItems: orderedItems.map((item) => ({
          foodId: Number(item.id) || null,
          foodName: item.name,
          quantity: preOrder[item.id],
      ...
      setSubmitError(error.response?.data?.message || 'Khong the tao dat ban. Vui long thu lai.');
    } finally {
      ...
      <div className="reservation-success">
        ...
        <div className="success-info">
          {createdReservation?.reservationCode && (
            <div className="info-row"><span>Ma dat ban:</span><strong>{createdReservation.reservationCode}</strong></div>
          )}
        ...
        <button className="btn-primary" onClick={() => { setSubmitted(false); setCreatedReservation(null); setStep(1); setPreOrder({}); setForm({ date:'',time:'',guests:2,name:'',phone:'',note:'' }); }}>
          Đặt bàn khác
        </button>
      </div>
    );
        ...
        <div className="confirm-step">
          <div className="confirm-layout">
            <div className="confirm-card card">
              ...
              {submitError && <p style={{ color: '#e53e3e', marginBottom: 12 }}>{submitError}</p>}
              <button className="reserve-btn" onClick={handleConfirm} disabled={submitLoading}>
                {submitLoading ? 'Dang tao dat ban...' : '✅ Xác nhận đặt bàn'}
              </button>
