...
import { getReviewsByFood } from '../../services/reviewService';
...
const FoodDetail = () => {
  ...
  const loadReviews = () => {
    getReviewsByFood(id).then(({ data }) => setReviews(data)).catch(() => {});
  };

  useEffect(() => {
    setLoading(true);
    Promise.all([getFoodById(id), getReviewsByFood(id)])
      ...
      <div className="bg-white rounded-3xl shadow-sm overflow-hidden mb-10">
        <div className="md:flex">
          <div className="md:w-1/2">
            <img src={food.image || 'https://placehold.co/800x500?text=Food'} alt={food.name} className="w-full h-72 md:h-full object-cover" />
          </div>
