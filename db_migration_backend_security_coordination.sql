...
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'ux_reservations_created_order_id'
    ...
    EXEC('CREATE UNIQUE INDEX ux_reservations_created_order_id ON reservations(created_order_id)
          WHERE created_order_id IS NOT NULL');

IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'ix_reservations_table_slot'
               AND object_id = OBJECT_ID('reservations'))
    EXEC('CREATE INDEX ix_reservations_table_slot
          ON reservations(assigned_table_id, start_at, end_at, status)');

IF COL_LENGTH('restaurant_tables', 'original_capacity') IS NULL
BEGIN
    ALTER TABLE restaurant_tables ADD original_capacity INT NULL;
    EXEC('UPDATE restaurant_tables SET original_capacity = capacity WHERE original_capacity IS NULL');
    EXEC('ALTER TABLE restaurant_tables ALTER COLUMN original_capacity INT NOT NULL');
END;

IF COL_LENGTH('restaurant_tables', 'version') IS NULL
BEGIN
    ALTER TABLE restaurant_tables ADD version BIGINT NULL;
    EXEC('UPDATE restaurant_tables SET version = 0 WHERE version IS NULL');
    EXEC('ALTER TABLE restaurant_tables ALTER COLUMN version BIGINT NOT NULL');
END;
